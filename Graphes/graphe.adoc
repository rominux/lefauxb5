...
1. [Bus uniquement] D_ArretBus -> E_ArretBus -> A_ArretBus
   - Prix : 12 + 20 = 32 €
2. [Mixte Bus+Train] D_ArretBus -> E_ArretBus -> (Correspondance) -> E_Gare -> A_Gare
   - Prix : 12 + 0 + 30 = 42 €
3. [Bus uniquement] D_ArretBus -> C_ArretBus -> B_ArretBus -> A_ArretBus
   - Prix : 15 + 25 + 10 = 50 €
4. [Mixte Train+Bus] D_Gare -> E_Gare -> (Correspondance) -> E_ArretBus -> A_ArretBus
   - Prix : 35 + 0 + 20 = 55 €
...
* **Type de graphe :** La situation réelle s'apparente à un multigraphe à arêtes typées. Cependant, notre astuce de modélisation consiste à représenter chaque infrastructure (gare, aéroport, arrêt de bus) comme un sommet distinct. Cela nous permet d'obtenir un graphe simple, non-orienté et valué.
